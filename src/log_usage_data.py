    ...
    if args.where == "Local":
        ...
        try:
            ...
            if args.manual == "yes":
                ...
                port = input("PORT:")
                engine_string = "{}://{}:{}@{}:{}/{}".format(conn_type, user, password, host, port, config.SQLALCHEMY_DATABASE_NAME)
                logging.debug("hehehe")
            else:
                logger.info('Creating an RDS database based on environment variables.')
                conn_type = "mysql+pymysql"
